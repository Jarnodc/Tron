...
namespace ImGui
{
    ...
    {
        auto status = PlotStatus::nothing;
        ...
        if (conf.values.ys != nullptr)
        {
            // draw only a single plot
        ...
        if (conf.values.count > 0)
        {
            ...
            if (conf.skip_small_lines)
                res_w = ImMin(static_cast<int>(conf.frame_size.x), conf.values.count);
            else
            ...
            if (conf.values.xs)
            {
                x_min = conf.values.xs[static_cast<size_t>(x_min)];
                x_max = conf.values.xs[static_cast<size_t>(x_max)];
            }
            ...
            const float inv_scale = (conf.scale.min == conf.scale.max)
                                        ? 0.0f
                                        : (1.0f / (conf.scale.max - conf.scale.min));
                ...
                case PlotConfig::Scale::Log10:
                    {
                        ...
                        while (start < x_max)
                        {
                            for (int i = 1; i < 10; ++i)
                            {
                                ...
                                if (x < x_min) continue;
                                if (x > x_max) break;
                                float t = static_cast<float>(log10(x / x_min)) / static_cast<float>(log10(x_max / x_min));
            ...
            for (int i = 0; i < ys_count; ++i)
            {
                ...
                auto tp0 = ImVec2(t0, 1.0f - ImSaturate((v0 - conf.scale.min) * inv_scale));
            ...
            if (conf.selection.show)
            {
                if (hovered)
                {
                    if (g.IO.MouseClicked[0])
                    {
                        ...
                        const int v_idx = cursor_to_idx(g.IO.MousePos, inner_bb, conf, x_min, x_max);
                        int start = conf.values.offset + (v_idx % conf.values.count);
                        int end = start;
                ...
                if (g.ActiveId == id)
                {
                    if (g.IO.MouseDown[0])
                    {
                        ...
                        int end = conf.values.offset + (v_idx % conf.values.count);
                        if (end > start)
                        {
                            if (conf.selection.sanitize_fn)
                                end = conf.selection.sanitize_fn(end - start) + start;
        ...
        if (conf.overlay_text)
            RenderTextClipped(ImVec2(frame_bb.Min.x, frame_bb.Min.y + style.FramePadding.y), frame_bb.Max,
                              conf.overlay_text, nullptr, nullptr, ImVec2(0.5f, 0.0f));
