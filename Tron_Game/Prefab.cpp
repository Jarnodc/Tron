...
std::shared_ptr<dae::GameObject> BlueTankPrefab(dae::Scene& scene)
{
	...
	pTurret->AddComponent(new SpriteComponent(pTurret.get(), SpriteComponent::SourcePart("TronSpriteSheet.png", 1, 1, { 32 * 6,32,32,32 }), { 0,0,32,32 }));
	pGO->AddChild(pTurret,true);
...
std::shared_ptr<dae::GameObject> BluePlayerTankPrefab(dae::Scene& scene)
{
	...
	dae::InputManager::GetInstance().AddCommand(SDL_SCANCODE_RSHIFT, std::make_shared<AttackCommand>(pGO.get()), kbID, dae::EInputState::Pressed);
	...
	dae::InputManager::GetInstance().AddCommand(SDL_SCANCODE_RCTRL, std::make_shared<RotateCommand>(pGO.get(), true), kbID, dae::EInputState::Down);
	...
	dae::InputManager::GetInstance().AddCommand(SDL_SCANCODE_SLASH, std::make_shared<RotateCommand>(pGO.get()), kbID, dae::EInputState::Down);
	...
	dae::InputManager::GetInstance().AddCommand(SDL_SCANCODE_LEFT, std::make_shared<MoveCommand>(pGO.get(), glm::vec3{ -1,0,0 }), kbID, dae::EInputState::Down);
	...
	dae::InputManager::GetInstance().AddCommand(SDL_SCANCODE_RIGHT, std::make_shared<MoveCommand>(pGO.get(), glm::vec3{ 1,0,0 }), kbID, dae::EInputState::Down);
	...
	dae::InputManager::GetInstance().AddCommand(SDL_SCANCODE_UP, std::make_shared<MoveCommand>(pGO.get(), glm::vec3{ 0,-1,0 }), kbID, dae::EInputState::Down);
	...
	dae::InputManager::GetInstance().AddCommand(SDL_SCANCODE_DOWN, std::make_shared<MoveCommand>(pGO.get(), glm::vec3{ 0,1,0 }), kbID, dae::EInputState::Down);
...
std::shared_ptr<dae::GameObject> RedTankPrefab(dae::Scene& scene)
{
	...
	if (scene.GetGameObject<ScoreComponent>().empty())
	{
	...
	{
		const auto scoreComp = scene.GetGameObject<ScoreComponent>().front()->GetComponent<ScoreComponent>();
		cont->GetSubject()->AddObserver(scoreComp);
	}
...
std::shared_ptr<dae::GameObject> GreenTankPrefab(dae::Scene& scene)
{
	...
	pGO->AddComponent(new BoxCollider(pGO.get(), 25));
	pGO->AddComponent(new SpriteComponent(pGO.get(), SpriteComponent::SourcePart("TronSpriteSheet.png", 1, 1, { 32 * 8,0,32,32 }), { 0,0,25,25 }));
	const auto tankComp{ new TankComponent(pGO.get(),{16,16,448,416},250) };
	pGO->AddComponent<MoveComponent>();
	pGO->AddComponent(new RigidBody(pGO.get(), { 20,20,20 }));
	pGO->AddComponent(tankComp);
	...
	const auto healthComp{ new HealthComponent(pHealth.get(), 1) };
	pHealth->AddComponent(healthComp);
	pHealth->AddComponent(new HorizontalSpriteList(pHealth.get(), new SpriteComponent(pHealth.get(), SpriteComponent::SourcePart("TronSpriteSheet.png", 1, 1, { 32 * 8,0,32,32 }), { 10,0,30,30 }), 3));
	pHealth->SetPosition(500, 60);
	healthComp->GetSubject()->AddObserver(cont);
	tankComp->GetSubject()->AddObserver(healthComp);
